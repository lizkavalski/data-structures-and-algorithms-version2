...
//Placement is IMPORTANT. The callback in  'speaker' is refering to 'greeting' in the test bececuse the code is look for the reference to the word, or value in that place.
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
//Again placement is important for this. Num is refering to a number, while arr is an array, then times is how long the array is, which is important in the case of the for-loop. The reason being it going to use the 'times' as to tell how many times to go through the loop, and what to do. In the for-loop it going to look at the callback (AddValuses) it going to put the number and repeat it a number of times in give an output in an array 
...
//In the removeOne the function is givin a num and based on the number if the number is divided by three and has a remainder of two it will be removed from the array. in the removeElement it will go through the array and see of the number is divideable by three and and if it has a reminder of two if does then it taken out, if it dosen't then it stays in the array 
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const removeWithForEach = (arr, callback) => {
  ...
  arr.forEach((curr,inx, arr)=>{
    return callback(curr,arr);
  });
  return arr;
}
...
const removeWithAnon = (arr) => {
  arr.forEach((element,inx,arr)=>{
    if(element % 3 === 2){
      arr.pop();
    }
    return (element,inx,arr);
  });
  return arr;
  }
...
const createList = (availableItems) => {
  ...
  let food =[];
  availableItems.forEach((items,idx,arr) => {
   if(items.available=== true){ 
    food.push(items.name);
    }
